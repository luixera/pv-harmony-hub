...
### Editar a legenda de um componente

**Duplo-clique no texto do desenho** seleciona o componente e põe o cursor no
campo "Legenda" do painel, com o conteúdo selecionado. Reposicionar o rótulo
continua sendo **arrastar**, e o botão "Voltar rótulo ao lugar" (só aparece
quando o rótulo foi movido) desfaz.

Antes, o duplo-clique *reposicionava* o rótulo — quem tentava corrigir "10A"
via o texto pular de lugar e concluía que não dava pra editar a legenda
(relato real, jul/2026). O gesto de duplo-clique num texto é editar; a função
de reposicionar virou botão, que é descobrível.

Detalhe de implementação: o foco **não** pode ser dado por
`requestAnimationFrame` — o campo só existe depois do commit do React, e o rAF
corre antes. É um `useState` + `useEffect` (`focusLegendPedido`).
