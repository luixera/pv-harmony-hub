...
async function buildValues(projectId: string): Promise<{ values: Record<string, string>; tenantId: string | null }> {
  ...
  const values: Record<string, string> = {
    ...
    // {endereco} é a junção das partes, igual ao gerador de documentos.
    endereco: [
      g?.address,
      g?.address_number ? `Nº ${g.address_number}` : '',
      g?.address_complement, g?.neighborhood,
      [g?.city, g?.state].filter(Boolean).join('/'),
      g?.cep ? `CEP: ${g.cep}` : '',
    ].filter(Boolean).join(', '),
    endereco_rua: g?.address ?? '', endereco_numero: g?.address_number ?? '',
    endereco_complemento: g?.address_complement ?? '', endereco_bairro: g?.neighborhood ?? '',
    endereco_cep: g?.cep ?? '', cep: g?.cep ?? '',
    cidade: g?.city ?? '', uf: g?.state ?? '',
